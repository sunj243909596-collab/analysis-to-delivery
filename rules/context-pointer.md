...
> 规范来源:`skills/disciplines/context-pointer`(已迁移至此)。旧的 `skills/disciplines/context-pointer/SKILL.md` 是本文件的兼容壳。

## 目的

项目级配置优先(读 `paths/*.md`),skill 级 config 仅作 fallback,严禁编造配置。
...
| 用户回答 / 项目状态 | 加载路径 |
|---|---|
| 项目根有 `paths/knowledge-path.md`,列了真实路径 | **Level 1** 直接读项目填的路径 |
| 项目根没有 `paths/*.md`,命中 skill 级 fallback | **Level 2** 用 `config/compliance/{行业}.md` 等 |
...
读项目根 paths/compliance-path.md → 加载引用的 GSP 法规文件
```
