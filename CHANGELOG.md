...
- 🆕 **thin root SKILL.md**:根 `SKILL.md` 缩减到 38 行(原 119 行),仅作 router / 架构图 / 加载规则。详细说明迁移到 `SPEC.md §13`。
- 🆕 **`rules/goal-boundary.md` + `scripts/goal-boundary-check.py`**:目标边界与分期硬门控 —— `TASK_CONFIRM` §二 / §三 + 可量化成功指标 + `PRD` §七 AC 阶段映射 + `TC` §三 阶段 + AC 关联 + `HANDOVER` §二 阶段达成 / 延后 / 差距。`P0/P1/P2` 优先级不当作阶段(防误判)。`tests/test_goal_boundary_check.py` 15 个 case。
