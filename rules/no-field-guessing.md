...
> 规范来源:`skills/disciplines/no-field-guessing`(已迁移至此)。旧的 `skills/disciplines/no-field-guessing/SKILL.md` 是本文件的兼容壳。

## 目的

数据库/接口字段必须从知识库或契约文档获取。看到需求中的"业务名称"就编造字段名 = 致命错误。

## 规则

### 1. 字段来源(优先级从高到低)

1. **项目级 `paths/knowledge-path.md`** 指向的真实知识库(测试库表结构、API 文档)
...
- ✅ 写任何 SQL / 数据模型 / API 契约前,先读 `paths/knowledge-path.md`
- ✅ 字段映射表必须与知识库定义一致
